...
public class Spielfeld {
	...
	public int shiftRow(int rowNr, boolean direction){
		...
		if(direction == false){
			...
			for (int i = 0; i < gameState.getWidth()-1; i++) {
				gameState.setCell(rowNr, i, gameState.getCell(rowNr, i+1)); //Shift all cells one cell to the left
			}
			gameState.setCell(rowNr, gameState.getWidth()-1, gameState.getOpNr()); //Overwrite the leftmost cell with the old opNr
		}
		else {
			opNr = gameState.getCell(rowNr, gameState.getWidth()-1); //Save the future opNr
			for (int i = gameState.getWidth()-1; i > 0; i--) {
				gameState.setCell(rowNr, i, gameState.getCell(rowNr, i-1)); //Shift all cells one cell to the right
			}
			gameState.setCell(rowNr, 0, gameState.getOpNr()); //Overwrite rightmost cell with old opNr
		}
	...
	public int shiftColumn(int columnNr, boolean direction){
		...
		if(direction == false){
			...
			for(int i = gameState.getHeight()-1; i>0; i--){
				gameState.setCell(i, columnNr, gameState.getCell(i-1, columnNr)); //Shift all cells one downwards
			}
